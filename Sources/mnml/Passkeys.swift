...
// If mnml dies meanwhile — quit, crash, killed — the agent never lets go of
// it, and refuses every passkey request mnml makes after that, before any
...
//
// So mnml carries the ceremony itself, as Chrome and Firefox do on the Mac,
...
final class Passkeys: NSObject {
    ...
    private static let log = Logger(subsystem: "com.farchan.mnml", category: "Passkeys")

    // MARK: - the Mac's permission

    /// Whether this Mac lets mnml use its passkeys at all: a question
    ...
    func perform(_ body: [String: Any], from caller: Caller, answer: @escaping ([String: Any]) -> Void) {
        ...
        // mnml itself behind doesn't get to bring up the Mac's sheet over
