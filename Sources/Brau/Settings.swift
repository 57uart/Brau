...
struct SettingsPanel: View {
    ...
    private var general: some View {
        Card {
            Line(
                ...
                isDefault ? "Brau is the default browser on this Mac" : "Mail, Slack and the rest still send links elsewhere"
            ) {
                if isDefault {
            ...
            Rule()
            Line("Open links from other apps in a small window", "To read and close, or keep with Open in Brau (⌘O)") {
            ...
            Rule()
            Line("Let a script drive Brau", "A local socket for testing. Its tabs open beside yours with a flask on them and never take over — see ./bench") {
    ...
    private var savingDetail: String {
        if #available(macOS 15.4, *), let name = Extensions.shared.passwordSavingTakenBy {
            return "\(name) does the saving — it asked Brau not to offer"
        }
        ...
        VStack(alignment: .leading, spacing: 18) {
            Card {
                ...
                if let trouble = shield.trouble {
                    Rule()
                    Line(trouble, "Nothing is being blocked until this clears — try again, or restart Brau") {
    ...
    private var about: some View {
        VStack(alignment: .leading, spacing: 18) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text("Brau")
                        ...
                        .foregroundStyle(Palette.ink)
                    Text("based on mnml by farchan and Search by Office Commun · version \(Updater.version)")
                        .font(.system(size: 12))
            ...
            Card {
                ...
                Rule()
                Line("Install updates on its own", "Off, Brau still looks once a day and tells you, and installs only when you press Install") {
    ...
    private var versionTitle: String {
        ...
        case .fetching(let next): return "Brau \(next.version) is downloading…"
        case .ready(let next): return "Brau \(next.version) is ready"
        case .offered(let next), .waiting(let next): return "Brau \(next.version) is out"
        }
    }
    ...
    private var versionDetail: String {
        ...
        case .ready(let next):
            return next.notes ?? "It's there the next time you open Brau"
