...
/// Brau's mark: B in Morse — dash, dot, dot, dot. A bare shape, like
...
struct Logomark: Shape {
    /// In a row, as on the app icon; or stacked, for a square spot such as
    /// the About page — the dash three dots wide over the three dots, the two
    /// rows centred in the square. Read left to right, top to bottom, it is
    /// still –···.
    var stacked = false

    /// Dots 90 across, a dash three dots long (Morse's own rule), 45 between.
    static let parts: [CGFloat] = [270, 90, 90, 90]
    ...
    private var pieces: [(CGFloat, CGFloat, CGFloat)] {
        if stacked { return [(0, 67.5, 360), (0, 202.5, 90), (135, 202.5, 90), (270, 202.5, 90)] }
        var x: CGFloat = 0
