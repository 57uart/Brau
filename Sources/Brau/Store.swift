...
enum Store {
    ...
    static var testing: Bool {
        if ProcessInfo.processInfo.environment["BRAU_PROBE"] != nil { return true }
        return Bundle.main.executablePath?.contains("/.build/") == true
    }

    /// Which test world a test run lives in. BRAU_PROBE=1, or a run from
    /// the build folder, is the test world, "Brau (test)". BRAU_PROBE=
    /// <name> is a world of its own, "Brau (<name>)", with settings and
    ...
    ///
    /// "Brau Test", the copy ./build.sh release test installs beside the real
    /// one, has a world of its own too ("Brau (copy)") — but is not a test
    ...
    static let world: String? = {
        ...
        let asked = (ProcessInfo.processInfo.environment["BRAU_PROBE"] ?? "").lowercased()
    ...
    static var testCopy: Bool { Bundle.main.bundleIdentifier == "com.57uart.brau.test" }

    /// A test run there to be weighed and timed rather than driven
    /// (BRAU_MEASURE beside BRAU_PROBE). It keeps what the shipped
    ...
    static var measuring: Bool {
        testing && ProcessInfo.processInfo.environment["BRAU_MEASURE"] != nil
    }
    ...
    static var ownContainer: Bool {
        (Bundle.main.bundleIdentifier ?? "") != "com.57uart.brau"
    }
    ...
    static let folder: URL = {
        ...
        let home = support.appendingPathComponent(world.map { "Brau (\($0))" } ?? "Brau", isDirectory: true)
        if world == nil {
    ...
    static let settings: UserDefaults = {
        ...
        let suite = world == "test" ? "com.57uart.brau.test" : "com.57uart.brau.test.\(world ?? "")"
