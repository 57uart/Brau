...
struct ContentView: View {
    ...
    /// A site used a key one of Brau's commands is on, and that command is
    ...
    private func shortcutAsking(_ ask: Browser.ShortcutAsk) -> some View {
        ...
        return HStack(spacing: 12) {
            Text("\(ask.site ?? "This page") used \(keys). Use Brau's \(command?.title ?? "shortcut") instead?")
                .font(.system(size: 12.5))
                .foregroundStyle(Palette.ink)
                .lineLimit(1)
            Button("Use Brau's") { browser.answerShortcutAsk(Brau: true) }
                .buttonStyle(.plain)
                .font(.system(size: 12))
                .foregroundStyle(Palette.ground)
                .padding(.horizontal, 11)
                .padding(.vertical, 5)
                .background(Palette.ink, in: Capsule())
            Button("Keep for websites") { browser.answerShortcutAsk(Brau: false) }
                .buttonStyle(.plain)
                .font(.system(size: 12))
                .foregroundStyle(Palette.muted)
    ...
    private func take(_ event: NSEvent) -> Bool {
        ...
        // A key handed to the page first, sent back unused: Brau's after all.
        ...
        // Brau acts only if the page sends it back unused.
    ...
    /// `log show --predicate 'subsystem == "com.57uart.brau"' --last 1h`.
    static let log = Logger(subsystem: "com.57uart.brau", category: "Switcher")
