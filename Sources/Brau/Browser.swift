...
final class Browser: NSObject, ObservableObject {
    ...
    /// A site just used a key Brau has a command on, and you asked to be
    ...
    func answerShortcutAsk(Brau: Bool) {
        guard let ask = shortcutAsk else { return }
        shortcuts.setConflict(Brau ? .BrauFirst : .websiteFirst, for: ask.id)
        shortcutAsk = nil
    }
    ...
    override init() {
        ...
        if prefs.bench {
            ...
            announce("“Let a script drive Brau” was turned on outside Settings, and stays off")
        }
    ...
    private func follow() {
        ...
        prefs.$bench
            ...
            .sink { [weak self] on in
                ...
                announce(on ? "Scripts can drive Brau — see ./bench" : "The bench is closed")
            }
            .store(in: &bag)
    ...
    /// General). Only one lifted this way goes home on its own when Brau
    /// comes back.
