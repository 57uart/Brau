...
final class Links: NSObject, NSApplicationDelegate {
    ...
    /// once Brau is the Mac's browser (it says it can open them, see
    ...
    /// while Brau was closed launched it with no window and the page
    ...
    static func writeFeedback() {
        // A new issue on Brau's repository, with the version already in it.
        var text = URLComponents(string: "https://github.com/57uart/Brau/issues/new")!
        text.queryItems = [
            URLQueryItem(name: "title", value: "Brau feedback — \(Updater.version) (\(Updater.build))"),
            URLQueryItem(name: "body", value: "\n\n—\nBrau \(Updater.version), build \(Updater.build), macOS \(ProcessInfo.processInfo.operatingSystemVersionString)"),
        ]
