...
// Brau doesn't have (its helper waits for an approval that never comes, and
// the popup spins). So Brau does the part it can: the first time you unlock
...
// Brau included — gets it without you. That keychain needs the access group
// only a copy signed with Brau's provisioning profile has; any other copy has
...
enum LockKey {
    ...
    (() => {
      ...
      if (field && !window.__brauLockKey) {
        window.__brauLockKey = true;
        document.addEventListener('input', (e) => {
          const t = e.target;
          if (t && t.type === 'password' && document.querySelector('[data-testid="lock-screen-submit-button"]')) window.__brauTyped = t.value;
        }, true);
      }
      const typed = field ? null : (window.__brauTyped || null);
      if (!field) window.__brauTyped = null;
    ...
    private static func offer(_ typed: String) {
        ...
        alert.informativeText = "Brau keeps your 1Password password in this Mac's keychain and types it in for you after Touch ID. Settings › Passwords forgets it."
    ...
    private nonisolated static var query: [String: Any] {
        [kSecClass as String: kSecClassGenericPassword,
         ...
         kSecAttrService as String: Store.testing ? "Brau 1Password unlock (test)" : "Brau 1Password unlock",
         kSecAttrAccount as String: "1Password"]
    }

    /// Whether this copy has somewhere to keep it: the data protection
    /// keychain, under Brau's own access group. Asked by writing, not by
    ...
    static let available: Bool = {
        let probe: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecUseDataProtectionKeychain as String: true,
            kSecAttrService as String: "Brau 1Password unlock (probe)",
    ...
    private static func keep(_ password: String) {
        ...
        item[kSecAttrLabel as String] = "Brau — 1Password unlock"
