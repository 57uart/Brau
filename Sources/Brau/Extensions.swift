...
final class Extensions: NSObject, ObservableObject {
    ...
    /// address they have in Chrome — Brau uses the same ids. Servers allow
    /// their own extension in by that origin (Raindrop's refuses any other),
    /// and sites look for an extension at it. WebKit's own
    /// webkit-extension:// is what Brau used before; addresses kept from
    ...
    private func load(_ item: Installed) async -> Bool {
        // The shim this build of Brau carries, in place of whatever the
    ...
    /// So does Brau, and then shows the page in the tab just opened.
    ...
    /// popup, and a popup that isn't in WebKit's own view (Brau's is its
    ...
    /// APIs Brau answers for it (history, bookmarks…) — an update that
    ...
    /// Chrome's own APIs, which Brau answers itself, and what each lets an
    /// extension do.
    ...
    static func describe(_ found: WKWebExtension, in folder: URL) -> [String] {
        var out: [String] = []
        // Leaving out what Brau itself added to the manifest.
        let added = Set((try? JSONSerialization.jsonObject(with: Data(contentsOf: folder.appendingPathComponent(".search-added")))) as? [String] ?? [])
        ...
        // Chrome's own, which Brau answers itself.
    ...
    /// permissions Brau answers itself.
    ...
    func press(_ id: String) {
        ...
        // Brau's lost the new popup's first messages to its worker.
...
extension Extensions: WKWebExtensionControllerDelegate {
    ...
    func webExtensionController(_ controller: WKWebExtensionController, promptForPermissionToAccess urls: Set<URL>, in tab: (any WKWebExtensionTab)?, for extensionContext: WKWebExtensionContext) async -> (Set<URL>, Date?) {
        ...
        // through permissions.request. So neither does Brau.
