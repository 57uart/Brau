...
enum Web {
    /// Where Brau's own page scripts run, and where their messages come
    ...
    /// secure" (24 Sep 2026). Only Brau's passkey patch has to stand in
    /// the page's own world, and nothing there leads back to it.
    @MainActor static let world = WKContentWorld.world(name: "Brau")

    /// Every handler of Brau's taken off a controller: in its own world,
...
final class Tab: ObservableObject, Identifiable {
    ...
    /// "Add to Brau" was pressed on the Chrome Web Store page this tab shows.
    ...
    /// The extension whose store page has its own "Add to Brau" button in
    ...
    func arm(hiding css: String) {
        ...
        // The store's "Add to Brau" only where Brau can add extensions.
        ...
        // functions — and reach Brau through a bridge in Brau's, off or on:
        ...
        if !FormRelay.passkeysOffered {
            ...
            // A site's passkey request is carried out by Brau itself: WebKit
...
final class PageView: WKWebView {
    ...
    /// A key the page sent back unused, offered to Brau's shortcuts first —
...
extension WKWebView {
    ...
    /// otherwise: an .html double-clicked in the Finder, once Brau is the
    ...
    /// JavaScript run in Brau's own world (see Web.world), where its page
