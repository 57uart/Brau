...
// If Brau dies meanwhile — quit, crash, killed — the agent never lets go of
// it, and refuses every passkey request Brau makes after that, before any
...
//
// So Brau carries the ceremony itself, as Chrome and Firefox do on the Mac,
...
// answer; what it leaves to Brau just waits, as it does while nobody picks
// one, never reaching macOS.
@MainActor
final class Passkeys: NSObject {
    static let shared = Passkeys()

    private static let log = Logger(subsystem: "com.57uart.brau", category: "Passkeys")

    // MARK: - the Mac's permission

    /// Whether this Mac lets Brau use its passkeys at all: a question
    ...
    func perform(_ body: [String: Any], from caller: Caller, answer: @escaping ([String: Any]) -> Void) {
        ...
        // Brau itself behind doesn't get to bring up the Mac's sheet over
...
final class PasskeyRelay: NSObject, WKScriptMessageHandlerWithReply {
    ...
    /// The page's word to Brau's side, and the answer back (see `bridge`).
    ...
    /// functions, and nothing of Brau's is there for a page to find — no
    /// window.webkit, no global of its own. Requests go to Brau's side as
    ...
    (function () {
      ...
      // ExtensionShims.passkeys) or Brau's own — and marked on the prototype
    ...
    /// Brau's side of it, in Brau's own world (Web.world), where the
