...
// "Switch to Chrome", and an "Add to Chrome" button that stays grey. Brau
...
// "Add to Brau" one — the button people already look for, rather than a bar
...
final class StoreRelay: NSObject, WKScriptMessageHandler {
    ...
    (function () {
      ...
      function render(ours) {
        ...
        label(ours, installed ? 'Added to Brau' : (busy ? 'Adding…' : 'Add to Brau'));
...
extension Browser {
    ...
    /// The page's "Add to Brau" was pressed: the extension this tab is showing.
    ...
    /// button can say "Added to Brau" or "Adding…".
