...
// ~/Library/Application Support/Brau, the defaults under
// com.57uart.brau, and the keychain. The session, the pins, the
...
final class Updater: ObservableObject {
    ...
    /// Where the file lives. BRAU_FEED, for a test run, points somewhere
    ...
    // ponytail: no public feed yet, so updates are off unless BRAU_FEED is set.
    // Put Brau's own appcast URL back here once there is one.
    // Only a test run reads it: the browser in use is never pointed at
    // another feed by how it happened to be started.
    static let feed: URL? = overridden ? ProcessInfo.processInfo.environment["BRAU_FEED"].flatMap(URL.init(string:)) : nil

    private static var overridden: Bool {
        Store.testing && ProcessInfo.processInfo.environment["BRAU_FEED"] != nil
    }
    ...
    func check(then done: @escaping (Release?) -> Void) {
        ...
        Task { [weak self] in
            ...
            case .none, .offered, .waiting:
                stage = .waiting(found)
                say?("Brau \(found.version) is out — it's in Settings")
            }
        }
    }
    ...
    private func landed(_ release: Release, worked: Bool) {
        ...
        say?(worked
            ? "Brau \(release.version) is ready — it's there the next time you open it"
            : "Brau \(release.version) is out — it's in Settings")
    }
    ...
    private static var reopen: [String] {
        ...
        for key in ["BRAU_PROBE", "BRAU_FEED"] {
...
private enum Swap {
    ...
    static func install(_ release: Updater.Release) async throws {
        ...
        let zip = scratch.appendingPathComponent("Brau.zip")
    ...
    /// `codesign -d -r-` prints for a shipped Brau: Apple's anchor, the
