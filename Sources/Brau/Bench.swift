...
final class Bench {
    ...
    /// keychain — its data protection half, where only apps signed as Brau
    ...
    enum Consent {
        private static var query: [String: Any] {
            [kSecClass as String: kSecClassGenericPassword,
             ...
             kSecAttrService as String: "com.57uart.brau.bench",
        ...
        /// without Brau's provisioning profile has no access group, and
    ...
    private func handle(_ request: [String: Any], _ given: @escaping ([String: Any]) -> Void) {
        ...
        case "select":
            ...
            // to someone using it: only on a BRAU_PROBE run.
        ...
        case "eval":
            ...
            house(tab)
            // Brau's own world is where its page scripts live (see Web.world).
        ...
        case "tap":
            ...
            // words. Only on a BRAU_PROBE run.
        ...
        case "shot":
            ...
            let path = (request["path"] as? String)
                ?? NSTemporaryDirectory() + "brau-bench-\(Bench.short(tab)).png"
        ...
        case "press":
            ...
            // `key` goes straight to a page instead. Only on a BRAU_PROBE run.
        ...
        case "key":
            ...
            // Only on a BRAU_PROBE run: it types into a page.
        ...
        case "resize":
            ...
            // bar. It moves the window, so only on a BRAU_PROBE run.
            guard Store.testing else {
        ...
        case "field":
            ...
            // and Core Animation's commit included. Only on a BRAU_PROBE run.
        ...
        case "bookmark":
            ...
            // until the run loop rests. Only on a BRAU_PROBE run.
        ...
        case "menu":
            ...
            // BRAU_PROBE run; nothing is drawn.
        ...
        case "keyeq":
            ...
            // Reports what Brau did and what the page saw. Only on a
            // SEARCH_PROBE run.
        ...
        case "window":
            ...
            guard let item = items.first(where: { $0.title == "Brau" }), let action = item.action else {
                answer(["error": "no Brau item in the Window menu", "items": items.map(\.title)])
                return
            }
