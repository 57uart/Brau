...
enum ExtensionShims {
    ...
    /// Brau's passkey patch, put first in every script an extension runs in
    ...
    nonisolated static let marker = "/* Brau: Chrome APIs WebKit lacks, filled in (ExtensionShims.swift) */"
    nonisolated static let ender = "/* Brau: end of shim */"
    ...
    /// `fresh`: a package just unpacked or copied in. What only Brau writes
    /// beside an extension — which permissions it added, which shim it
    /// carries — is Brau's to say, never the package's: anything by those
    ...
    nonisolated static func prepare(_ folder: URL, fresh: Bool = false) throws {
        ...
        // described by what it asked for, not by what Brau gave it.
            ...
            if let worker = background["service_worker"] as? String,
               let path = inside(worker, of: folder) {
                if var source = try? String(contentsOf: path, encoding: .utf8) {
                    ...
                    // Brau puts its newer shim in its place.
        ...
        // that runs in the page's own world has Brau's passkey patch before
        // it: a password manager's there keeps a reference to
        // navigator.credentials as it finds it, and that has to be Brau's,
    ...
    (() => {
      ...
      // (There, Brau's passkey patch holds navigator.credentials.)
      ...
      // 1Password's asks its worker first, so its click always is. Brau
      ...
      // one — Safari's, as Brau's tabs send — not the Chrome one the
      ...
      if (runtime && typeof runtime.sendMessage === "function") {
        ...
        put(runtime, "sendMessage", (...args) => {
          ...
          // hear it and still allowed to. Brau writes it here instead, the
      ...
      const refuse = (what) => (...args) => {
        const callback = args.length && typeof args[args.length - 1] === "function" ? args.pop() : null;
        const error = new Error(what + " isn't available in Brau");
      ...
      // shown in Brau, so there is nothing for them to do.
      ...
      // The popup Brau shows is a page of its own, known to WebKit as a
      ...
      // Brau opens a popup itself (see Extensions.press), and has to know
      ...
      // rest still go in: one rule Brau can't honour shouldn't cost an
      ...
      // Context menu entries for places Brau has no menu for — the old
      ...
      // no group — Brau has none), which code tests before anything else;
      ...
      // where Chrome answers false. The ones Brau answers itself are
      // Brau's to grant: those a manifest names are granted, optional
      // ones are asked for.
      ...
      // What an extension registers for a page's own world has Brau's
    ...
    static func answer(_ message: Any, from context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        guard let body = message as? [String: Any], let api = body["api"] as? String else {
            return ["error": "Not a Brau message"]
        }
    ...
    private static func run(_ api: String, _ args: [Any], context: WKWebExtensionContext, owner: Extensions) async throws -> Any? {
        ...
        case "downloads.pause", "downloads.resume", "downloads.cancel", "downloads.removeFile", "downloads.getFileIcon":
            throw Unsupported(what: "\(api) isn't available in Brau yet")
        ...
        // MARK: permissions Brau grants itself
        ...
        case "power.requestKeepAwake":
            ...
            if IOPMAssertionCreateWithName(kind, IOPMAssertionLevel(kIOPMAssertionLevelOn), "An extension in Brau" as CFString, &assertion) == kIOReturnSuccess {
                awake[id] = assertion
            }
            return nil
        ...
        case "power.reportActivity":
            var assertion: IOPMAssertionID = 0
            IOPMAssertionDeclareUserActivity("An extension in Brau" as CFString, kIOPMUserActiveLocal, &assertion)
            return nil
        ...
        case "readingList.addEntry", "readingList.removeEntry", "readingList.updateEntry":
            throw Unsupported(what: "Brau has no reading list")
        ...
        case "tabGroups.get", "tabGroups.update", "tabGroups.move":
            throw Unsupported(what: "Brau has no tab groups")
        ...
        default:
            throw Unsupported(what: "\(api) isn't available in Brau")
        }
    }
    ...
    static func userScriptFile(_ script: [String: Any], in folder: URL) throws -> String {
        ...
        """#
        let text = #"""
        /* Brau: a user script (chrome.userScripts) */
        search_user_script: {
    ...
    /// launches as Chrome keeps it. Brau acts on one of them — an
    ...
    /// chrome.browsingData, from what WebKit and Brau keep.
