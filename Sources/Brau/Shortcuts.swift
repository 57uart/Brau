...
// Every command Brau has a key for, in one list: the menus draw from it, the
...
/// Who gets a key that Brau and the page both want.
enum Conflict: String, Codable, CaseIterable, Identifiable {
    case BrauFirst, websiteFirst, prompt
    var id: String { rawValue }
    var title: String {
        switch self {
        case .BrauFirst: return "Use Brau First"
...
/// Something Brau does, and the key it does it on unless told otherwise.
struct Command: Identifiable {
    enum Section: String, CaseIterable {
        case app = "Brau", file = "File", edit = "Edit", view = "View", tabs = "Tabs", bookmarks = "Bookmarks", history = "History"
    }
...
final class ShortcutStore: ObservableObject {
    ...
    func conflict(for id: String) -> Conflict { conflicts[id] ?? .BrauFirst }
    ...
    func setConflict(_ conflict: Conflict, for id: String) {
        conflicts[id] = conflict == .BrauFirst ? nil : conflict
        save()
    }

    func setAllConflicts(_ conflict: Conflict) {
        conflicts = conflict == .BrauFirst ? [:] : Dictionary(uniqueKeysWithValues: Command.all.map { ($0.id, conflict) })
        save()
    }
...
/// Which of Brau and the page a shortcut goes to first.
enum KeyRoute: Equatable {
    /// Brau acts, the page never sees the key.
    case run
    /// The page gets it; Brau acts only if the page sends it back unused.
    case hand
}

extension KeyRoute {
    static func decide(_ conflict: Conflict, pageHasFocus: Bool) -> KeyRoute {
        pageHasFocus && conflict != .BrauFirst ? .hand : .run
    }
}

/// A key handed to the page for it to use first, remembered until the page
/// either sends it back — then Brau acts on it — or keeps it.
@MainActor
