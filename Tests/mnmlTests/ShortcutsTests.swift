...
final class ShortcutsTests: XCTestCase {
    ...
    override func setUp() {
        defaults = UserDefaults(suiteName: "mnml.tests.shortcuts.\(UUID().uuidString)")
    }
    ...
    func testOverridesDisableAndReset() {
        ...
        XCTAssertEqual(store.key(for: "file.newTab"), KeyCombo("t"))
        XCTAssertEqual(store.conflict(for: "file.newTab"), .mnmlFirst)
        XCTAssertFalse(store.isModified("file.newTab"))
    ...
    func testConflicts() {
        ...
        store.setAllConflicts(.mnmlFirst)
        XCTAssertFalse(store.anyModified)

        XCTAssertEqual(KeyRoute.decide(.mnmlFirst, pageHasFocus: true), .run)
