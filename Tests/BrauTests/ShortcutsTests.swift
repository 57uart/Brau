...
final class ShortcutsTests: XCTestCase {
    ...
    override func setUp() {
        defaults = UserDefaults(suiteName: "Brau.tests.shortcuts.\(UUID().uuidString)")
    }
    ...
    func testOverridesDisableAndReset() {
        ...
        XCTAssertEqual(store.key(for: "file.newTab"), KeyCombo("t"))
        XCTAssertEqual(store.conflict(for: "file.newTab"), .BrauFirst)
        XCTAssertFalse(store.isModified("file.newTab"))
    ...
    func testConflicts() {
        ...
        store.setAllConflicts(.BrauFirst)
        XCTAssertFalse(store.anyModified)

        XCTAssertEqual(KeyRoute.decide(.BrauFirst, pageHasFocus: true), .run)
