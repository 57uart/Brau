...
name: brau-bench
description: >
  Drive the Brau macOS browser from the repo-root ./bench command: open
  ...
  press Chrome extensions. Use when the user asks to test Brau, drive the
  browser, run ./bench, open a page in Brau, screenshot a tab, check a
  panel, or exercise an extension, and when they run /brau-bench.
metadata:
  short-description: Drive Brau with ./bench
---

# Brau bench

Brau is a native Mac browser in this repo. Drive the running app with `./bench` from the repo root. `./bench help` is the command syntax. This file is how to use it without touching the browser a person has open.
...
|---|---|---|
| `--test` | World `test` | `~/Library/Application Support/Brau (test)/` |
| `--world NAME` | World `NAME` (lowercase letters, digits, hyphens) | `~/Library/Application Support/Brau (NAME)/` |
| none | The installed browser they actually use | `~/Library/Application Support/Brau/` |

Use a test world for any work that changes chrome, installs or removes extensions, resizes, sends real key events, or selects a tab. A `swift build` binary under `.build/` is always world `test`, even with no `BRAU_PROBE`. `./fresh.sh` launches `build/Brau.app` with `BRAU_PROBE` set, which is also a test world.
...
One process per world. Quit a process only after its executable path is this repo's `build/Brau.app` or a `.build/` binary, or its environment contains `BRAU_PROBE`. Leave `/Applications/Brau.app` alone. `killall`, quitting by the name Brau, and `osascript` quit hit the installed app too: same bundle id, same process name.

1. `./bench --test tabs` (or `--world NAME`). A tab list means that world is listening. Do not launch another.
2. If it prints `Brau isn't listening`:
   - Nothing running for that world: `defaults write SUITE bench -bool true`, then `./fresh.sh again`. For a named world, `BRAU_PROBE=NAME ./fresh.sh again`.
...
4. The suite is `com.57uart.brau.test`, or `com.57uart.brau.test.NAME` for a named world.
...
If `./bench tabs` (no flag) is not listening, ask them to turn on **Settings › General › Let a script drive Brau**. Do not write defaults for the installed app.

`./fresh.sh` builds `build/Brau.app` when that bundle is missing.
...
./bench --test text "$id"
./bench --test shot "$id" /tmp/Brau-bench.png
./bench --test close "$id"
```
