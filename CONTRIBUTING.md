...
Brau stays small by default. Anything new that changes how the browser
...
- [ROADMAP.md](ROADMAP.md) and [CHANGELOG.md](CHANGELOG.md) are Search's, kept as they came from upstream so its changes keep merging cleanly. Brau's own changes are described in its commits and pull requests.
...
- Anything that phones home, adds analytics, or changes what leaves the app over the network — see “Privacy, concretely” in [README.md](README.md) for what leaves the Mac today.
- Vendoring Chromium or any other engine. This is a WebKit browser on purpose.

## Review

Pull requests are reviewed by [@57uart](https://github.com/57uart). This is a personal project, so an answer may take a while; pinging a stale pull request after a couple of weeks is fine.

## Reporting a bug

Open an issue with: what you did, what you expected, what happened instead, and your macOS version. A crash log, if there is one, lives at `~/Library/Application Support/Brau/crash.log` — it only ever stays on your Mac unless you paste it into the issue yourself.
